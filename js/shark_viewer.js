var SharkViewer = function (parameters) {
   ...
   */
   ...
   //    0xF08080,
   //    0x20B2AA,
   //    0x6A5ACD, //RACA  0x00008B
   //    0x20B2AA, //RMCA 0xB22222
   //    0xB22222, //LMCA
   //    0x00008B, //LACA
   //    0x228B22, 
   //    0xFFD700,
   // ];
     this.colors = [
      0xfdfdfd,
      0xD3D3D3,
      0xD3D3D3, //RACA  0x00008B
      0xD3D3D3, //RMCA 0xB22222
      0xD3D3D3, //LMCA
      0xD3D3D3, //LACA
      0xD3D3D3, 
      0xD3D3D3,
...
SharkViewer.prototype.calculateCameraPosition = function (fov) {
   ...
   var z = (max_bb / (Math.tan(fov * (Math.PI / 180.0) / 2) * 2)) * 1.50;
   return z;
};
...
SharkViewer.prototype.init = function () {
   ...
   for (var color in this.colors) {
      if (this.colors.hasOwnProperty(color)) {
         var material = new THREE.MeshLambertMaterial({color: this.colors[color],transparent:true,opacity:0});
         console.log(material)
         material.opacity = 0.2;
         this.three_materials.push(material);
      }
   }
   ...
   if (this.mode === 'skeleton' || this.show_cones === false) {
      this.material = new THREE.LineBasicMaterial({color: this.colors[this.colors.length - 1],linewidth:50});
      ...
      for (var node in this.swc) {
         if (this.swc.hasOwnProperty(node)) {
            if (this.swc[node].parent != -1) 
            {
               ...
               vertexSkelton.push(new THREE.Vector3( node.x, node.y, node.z ))
            }
         }
      }
   ...
   //this.neuron.rotation.x = Math.PI / 2;
...
// animation loop
SharkViewer.prototype.animate = function (boolAnimate, direction) {

   if(boolAnimate){
   var xAxis = new THREE.Vector3(0,1,0);
   var point = new THREE.Vector3(0,-10,0);
  
   rotateAboutPoint(this.neuron,point , xAxis, -0.005);
   ...
   // - see details at http://my.opera.com/emoller/blog/2011/12/20/requestanimationframe-for-smart-er-animating
      ...
      id = requestAnimationFrame(this.animate.bind(this));


   
   //this.rotation -= 0.01 * 2;
   //this.neuron.rotation.x = Math.PI / 2;
   //this.neuron.rotation.y -= 0.001;
   //this.neuron.rotation.z += 0


  // rotateAroundWorldAxis(this.neuron, xAxis, -0.01);
   //var zAxis = new THREE.Vector3(0,0,1);
   //rotateAroundWorldAxis(this.neuron, zAxis, +0.01);
   //rotateAroundObjectAxis(this.neuron,'y',0.001)
...
//Copy of animation loop

// animation loop
SharkViewer.prototype.animate1 = function (boolAnimate, direction) {
   ...
   // - see details at http://my.opera.com/emoller/blog/2011/12/20/requestanimationframe-for-smart-er-animating
      ...
      id = requestAnimationFrame(this.animate1.bind(this));
...
SharkViewer.prototype.test = function()
{
